...
//     }
//     public int memoization(int index , int buy , int[] prices){
//         int n = prices.length ;
//         int profit = 0 ;
//         //base case 
//         if(index >= n) return 0 ;
...
//         }else{
//             int Selled = prices[index] + memoization(index + 2 , 1 , prices);       //take case
...
//             profit = Math.max(Bought , notBought);
//         }else{
//             int Selled = prices[index] + memoization(index + 2 , 1 , prices , dp);       //take case
...
//                     profit = Math.max(Bought , notBought);
//                 }else{
//                     int Selled = prices[index] + dp[index + 2][1] ;       //take case
...
class Solution {
    public int maxProfit(int[] prices) {
        ...
        int[] ahead1 = new int[2]; // dp[index + 1]
        int[] ahead2 = new int[2]; // dp[index + 2]
        int[] curr = new int[2];
        ...
        for (int index = n - 1; index >= 0; index--) {
            ...
            // buy = 1
            curr[1] = Math.max(
                    -prices[index] + ahead1[0],
                    ahead1[1]
            );

            // buy = 0
            curr[0] = Math.max(
                    prices[index] + ahead2[1], // cooldown
                    ahead1[0]
            );
